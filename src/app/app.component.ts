...
export class AppComponent implements OnInit {
  ...
  numReplans: number = 0
  seriesName: string = ''
                    // For lower grapn on MD-user page; workdays2 has data for this 
   ...
   this .userid = params['userid']
    ...
    this .getWFdataByStartDate(dates).subscribe(res =>{   //getPlansByStartDate.php  -  uses dev version"
      ...
      this .plans = rData[0][0]
          console.log("5454 %o", this .theData)    
      this .initializeCTSimToContComp()
      this .makeEmptyDist()    
      this .makeEmptyTimingDist()
      console.log("105105 amdstats %o   CTSimt %o", this .AMDStats, this .CTSimToContComp)
...
makeEmptyDist(){
   let i = 0
   console.log("9393 %o", this .theData)    
   if ( this .AMDStats  )
...
makeEmptyTimingDist(){
  ...
  populateByPassFail(){
    ...
    // Skip specified Modalities
      ...
      console.log("201201 %o", this .AMDStats)      
    }  
/**
...
setOptions(){
   this .options = 
   {
    ...
    title: {  text: 'Contour-Completion to Start Date '},
    subtitle :{text: "Data Collected from "+ this.collectionDates[0] + " to " + this.collectionDates[1] + ',  ' + this.numUrgent + ' Urgent plans and ' + this.numReplans + ' Replans omitted'},
    xAxis: { 
...
setOptions2(){
   if (this .AMDStats[0] &&  this .AMDStats[0].MDLastName)
      this .seriesName = 'Days for Dr. ' + this .AMDStats[0].MDLastName
   let xTics = Array()
   ...
   this .options2 = 
   {
     ...
     title: {
       text:"CT-Sim to Contour Completion "
     },
     subtitle: {
       text: ''
     },
  ...
  getWFdataByStartDate(dates?:any): Observable<wfData>{  
    ...
    if (isDevMode())
      url =  "https://whiteboard.partners.org/esb/FLwbe/QAdashBd/dev/getPlansByStartDate.php";  
